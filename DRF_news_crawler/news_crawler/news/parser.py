...
def scraper(query):
    ...
    for url in urls:
        ...
        for i in reaction["contents"][0]["reactionMap"]:
            ...
            news[reaction_name] = int(reaction_num)
...
if __name__ == '__main__':
    ...
    for item in data:
        News(title=item["title"], body=item["body"], press=item["press"], journalist=item["journalist"],
             comment_num=item["comment_num"], angry=item["angry"], warm=item["warm"], like=item["like"],
             want=item["want"], sad=item["sad"]).save()
